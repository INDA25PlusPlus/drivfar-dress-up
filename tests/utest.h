...
#define UTEST_STRNCMP(x, y, size)                                          \
	_Pragma("clang diagnostic push") _Pragma(                          \
		"clang diagnostic ignored \"-Wdisabled-macro-expansion\"") \
...
#define UTEST_COND(x, y, cond, msg, is_assert)                                          \
	...
	{                                                                               \
		...
		if (!((xEval)cond(yEval))) {                                            \
			...
			UTEST_PRINTF("%s) " #cond " (%s", xAsString,                    \
				     yAsString);                                        \
			UTEST_PRINTF(")\n");                                            \
			UTEST_PRINTF("    Actual : ");                                  \
...
#define UTEST_COND(x, y, cond, msg, is_assert)                         \
	...
	{                                                              \
		if (!((x)cond(y))) {                                   \
			UTEST_PRINTF("%s:%i: Failure (Expected " #cond \
				     " Actual)",                       \
				     __FILE__, __LINE__);              \
			if (strlen(msg) > 0) {                         \
	...
	{                                                                     \
		...
		if (0 != memcmp(xEval, yEval, sizeEval)) {                    \
			...
			for (i = 0; i < sizeEval; ++i) {                      \
				const unsigned char b = UTEST_PTR_CAST(       \
					const unsigned char *, xEval)[i];     \
				UTEST_PRINTF("%02X ", b);                     \
			}                                                     \
			UTEST_PRINTF("\n");                                   \
			UTEST_PRINTF("    Actual : ");                        \
...
#define UTEST_NEAR(x, y, epsilon, msg, is_assert)                              \
	UTEST_SURPRESS_WARNING_BEGIN do                                        \
	{                                                                      \
		const double diff = utest_fabs(UTEST_CAST(double, x) -         \
					       UTEST_CAST(double, y));         \
	...
	{                                                                      \
		...
		try {                                                          \
			...
			if (0 != UTEST_STRNCMP(what, exception_message,        \
				...
				message_caught = UTEST_PTR_CAST(               \
					char *, malloc(message_size));         \
				UTEST_STRNCPY(message_caught, what,            \
					      message_size);                   \
			}                                                      \
		} catch (...) {                                                \
			exception_caught = 2;                                  \
		...
		if (1 != exception_caught) {                                   \
			...
			*utest_result = UTEST_TEST_FAILURE;                    \
			...
			UTEST_PRINTF("%s:%i: Failure\n", __FILE__, __LINE__);  \
			UTEST_PRINTF(                                          \
				"  Expected : %s exception with message %s\n", \
...
#define EXPECT_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message)    \
	UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, "", \
				     0)
...
#define UTEST(SET, NAME)                                                       \
	...
	static void utest_##SET##_##NAME(int *utest_result,                    \
					 size_t utest_index)                   \
	{                                                                      \
		(void)utest_index;                                             \
...
#define UTEST_I(FIXTURE, NAME, INDEX)                                          \
	...
	{                                                                      \
		...
		utest_run_##FIXTURE##_##NAME##_##INDEX(utest_result,           \
						       &fixture);              \
	...
	{                                                                      \
		...
		for (i = 0; i < (INDEX); i++) {                                \
			...
			char *name =                                           \
				UTEST_PTR_CAST(char *, malloc(name_size));     \
			utest_state.tests = UTEST_PTR_CAST(                    \
				struct utest_test_state_s *,                   \
				utest_realloc(                                 \
					UTEST_PTR_CAST(void *,                 \
			...
			if (utest_state.tests && name) {                       \
				...
				UTEST_SNPRINTF(name, name_size,                \
					       "%s/%" UTEST_PRIu64, name_part, \
					       iUp);                           \
			} else {                                               \
				if (utest_state.tests) {                       \
					free(utest_state.tests);               \
	...
	UTEST_SURPRESS_WARNINGS_END                                            \
	void utest_run_##FIXTURE##_##NAME##_##INDEX(                           \
...
int utest_main(int argc, const char *const argv[])
{
	...
	for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
		...
		if (0 ==
		    UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
			...
			goto cleanup;
		} else if (0 == UTEST_STRNCMP(argv[index], filter_str,
					      strlen(filter_str))) {
			/* user wants to filter what test cases run! */
			filter = argv[index] + strlen(filter_str);
		} else if (0 == UTEST_STRNCMP(argv[index], output_str,
					      strlen(output_str))) {
			utest_state.output = utest_fopen(
		...
		} else if (0 == UTEST_STRNCMP(
					argv[index], random_order_with_seed_str,
	...
	if (random_order) {
		...
		for (index = utest_state.tests_length; index > 1; index--) {
			...
			const utest_uint32_t next =
				((word >> 22u) ^ word) %
				UTEST_CAST(utest_uint32_t, index);
	...
	for (index = 0; index < utest_state.tests_length; index++) {
		...
		try {
			utest_state.tests[index].func(
				&result, utest_state.tests[index].index);
