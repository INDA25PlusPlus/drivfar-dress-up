...
#define CLAY_SIZING_FIXED(fixedSize)                    \
    (CLAY__INIT(Clay_SizingAxis){                       \
        .size = { .minMax = { fixedSize, fixedSize } }, \
...
#define CLAY_TEXT(text, ...)                                                 \
    Clay__OpenTextElement(text, CLAY__CONFIG_WRAPPER(Clay_TextElementConfig, \
                                                     __VA_ARGS__))
...
typedef struct Clay_ElementId {
    ...
    uint32_t
        offset; // A numerical offset applied after computing the hash from stringId.
    uint32_t
        baseId; // A base hash value to start from, for example the parent element ID is used when calculating CLAY_ID_LOCAL().
...
typedef struct Clay_SizingAxis {
    union {
        Clay_SizingMinMax
            minMax; // Controls the minimum and maximum size in pixels that this element is allowed to grow or shrink to, overriding sizing types such as FIT or GROW.
    ...
    Clay__SizingType
        type; // Controls how the element takes up space inside its parent container.
} Clay_SizingAxis;
...
typedef struct Clay_LayoutConfig {
    Clay_Sizing
        sizing; // Controls the sizing of this element inside it's parent container, including FIT, GROW, PERCENT and FIXED sizing.
    Clay_Padding
        padding; // Controls "padding" in pixels, which is a gap between the bounding box of this element and where its children will be placed.
    uint16_t
        childGap; // Controls the gap in pixels between child elements along the layout axis (horizontal gap for LEFT_TO_RIGHT, vertical gap for TOP_TO_BOTTOM).
    Clay_ChildAlignment
...
typedef struct Clay_ClipElementConfig {
    ...
    Clay_Vector2
        childOffset; // Offsets the x,y positions of all child elements. Used primarily for scrolling containers.
...
typedef struct Clay_BorderElementConfig {
    Clay_Color
        color; // Controls the color of all borders with width > 0. Conventionally represented as 0-255, but interpretation is up to the renderer.
    Clay_BorderWidth
        width; // Controls the widths of individual borders. At least one of these should be > 0 for a BORDER render command to be generated.
...
typedef enum {
    ...
    CLAY_TRANSITION_PROPERTY_DIMENSIONS = CLAY_TRANSITION_PROPERTY_WIDTH |
                                          CLAY_TRANSITION_PROPERTY_HEIGHT,
    CLAY_TRANSITION_PROPERTY_BOUNDING_BOX = CLAY_TRANSITION_PROPERTY_POSITION |
                                          ...
                                          Clay_String string)
{
    ...
    return CLAY__INIT(
        Clay_String){ .length = string.length,
...
Clay_ElementId Clay__HashString(Clay_String key, const uint32_t seed)
{
    ...
    hash += (hash << 15);
    return CLAY__INIT(Clay_ElementId){
        .id = hash + 1, .offset = 0, .baseId = hash + 1, .stringId = key
    }; // Reserve the hash result of zero as "null id"
}
    ...
    while (
        ...
        if (hashItem->elementId.id ==
            ...
            if (hashItem->generation <=
                context
                    ->generation) { // First collision - assume this is the "same" element
                hashItem->appearedThisFrame = hashItem->generation <
                                              context->generation;
                hashItem->elementId =
...
void Clay__CloseElement(void)
{
    ...
    bool elementIsFloating = openLayoutElement->config.floating.attachTo !=
                             CLAY_ATTACH_TO_NONE;
...
void Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig textConfig)
{
    ...
    textElement->minDimensions =
        CLAY__INIT(Clay_Dimensions){ .width = textMeasured->minWidth,
                                   ...
                                   Clay__int32_tArray *aspectRatioElementsOut)
{
    ...
    for (int32_t rootIndex = 0;
        ...
        for (int32_t i = 0; i < bfsBuffer.length; ++i) {
            ...
            float parentSize = xAxis ? parent->dimensions.width :
            ...
            bool sizingAlongAxis =
                ...
                (!xAxis &&
                 parentLayoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM);
            ...
            for (int32_t childOffset = 0; childOffset < parent->children.length;
                ...
                int32_t childElementIndex =
                    parent->children.elements[childOffset];
                ...
                if (aspectRatioElementsOut &&
                    ...
                    Clay__int32_tArray_Add(aspectRatioElementsOut,
                                           childElementIndex);
                }
                    ...
                    while (sizeToDistribute > CLAY__EPSILON &&
                        ...
                        for (int childIndex = 0;
                            ...
                            float *childSize = xAxis ?
                                                   &child->dimensions.width :
                                                   &child->dimensions.height;
                            float maxSize = xAxis ? child->config.layout.sizing
                                                        .width.size.minMax.max :
                                                    child->config.layout.sizing
                                ...
                                bool generateRenderCommands)
{
    ...
    Clay__int32_tArray aspectRatioElements =
        context->reusableElementIndexBuffer;
    ...
    for (int32_t textElementIndex = 0; textElementIndex < textElements.length;
        ...
        Clay__MeasureTextCacheItem *measureTextCacheItem =
            Clay__MeasureTextCached(&textElementData->text,
        ...
        while (wordIndex != -1) {
            ...
            // measuredWord->length == 0 means a newline character
                ...
                lineWidth += measuredWord->width +
                             containerElement->textConfig.letterSpacing;
    ...
    while (dfsBuffer.length > 0) {
        ...
        if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
            ...
            for (int32_t j = 0; j < currentElement->children.length; ++j) {
                Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
                    &context->layoutElements,
                    currentElement->children.elements[j]);
    ...
    for (int32_t rootIndex = 0;
            ...
            if (clipHashMapItem &&
                ...
                if (context->externalScrollHandlingEnabled) {
                    if (clipHashMapItem->layoutElement->config.clip.horizontal) {
                        rootPosition.x += clipHashMapItem->layoutElement->config
                                              .clip.childOffset.x;
                    }
                    if (clipHashMapItem->layoutElement->config.clip.vertical) {
                        rootPosition.y += clipHashMapItem->layoutElement->config
                                              .clip.childOffset.y;
                    }
                }
                if (generateRenderCommands) {
                    Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                        ...
                        .userData = 0,
                        .id =
                            Clay__HashNumber(rootElement->id,
        ...
        while (dfsBuffer.length > 0) {
                ...
                if (generateRenderCommands &&
                    ...
                    if (Clay__BorderHasAnyWidth(
                        ...
                        if (borderConfig->width.betweenChildren > 0 &&
                            ...
                            float halfWidth =
                                borderConfig->width.betweenChildren / 2;
                            ...
                            if (layoutConfig->layoutDirection ==
                                CLAY_LEFT_TO_RIGHT) {
                                for (int32_t i = 0;
                                     i < currentElement->children.length; ++i) {
                                    Clay_LayoutElement *childElement =
                                        Clay_LayoutElementArray_Get(
                                            &context->layoutElements,
            ...
            if (useStoredBoundingBoxes &&
                        ...
                        if (transitionData->state !=
                            ...
                            if ((currentElement->config.transition.properties &
                                ...
                                currentElementBoundingBox.width =
                                    transitionData->currentState.boundingBox
                                        .width;
                            if ((currentElement->config.transition.properties &
                ...
                if (!found &&
                    currentElement->config.transition.exit.setFinalState) {
                    dfsBuffer.length--;
                    continue;
                }
            }
            if (currentElement->config.floating.attachTo !=
                CLAY_ATTACH_TO_NONE) {
                Clay_FloatingElementConfig *floatingElementConfig =
            ...
            if (generateRenderCommands && !offscreen) {
                if (currentElement->isTextElement) {
                    ...
                    float naturalLineHeight = currentElement->textElementData
            ...
            if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                for (int32_t i = 0; i < currentElement->children.length; ++i) {
                    ...
                    contentSizeCurrent.height =
                        CLAY__MAX(contentSizeCurrent.height,
                                  childElement->dimensions.height);
                }
                ...
                for (int32_t i = 0; i < currentElement->children.length; ++i) {
                    ...
                    if (childElement->exiting)
                        continue;
                    contentSizeCurrent.width =
                        CLAY__MAX(contentSizeCurrent.width,
                                  childElement->dimensions.width);
                    contentSizeCurrent.height +=
                        childElement->dimensions.height;
                }
                ...
                dfsBuffer.internalArray[newNodeIndex] =
                    CLAY__INIT(Clay__LayoutElementTreeNode){
                        ...
                        .position = CLAY__INIT(Clay_Vector2){ childPosition.x,
                                                              childPosition.y },
                        .nextChildOffset = { .x = (float)childElement->config
                                                      .layout.padding.left,
                                             .y = (float)childElement->config
                                                      .layout.padding.top },
                    };
                                        ...
                                        bool offscreen)
{
    ...
    {
        CLAY_TEXT(label,
                  CLAY_TEXT_CONFIG({ .textColor = offscreen ?
                                    ...
                                    int32_t highlightedRowIndex)
{
    ...
    for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (currentElement
                    ->exiting) { // TODO there is a duplicate ID problem with exiting elements
                dfsBuffer.length--;
                continue;
            }
                 ...
                 { .layout = Clay__DebugView_ScrollViewItemLayoutConfig })
            {
                ...
                if (currentElement->isTextElement) {
                    Clay__RenderElementConfigTypeLabel(
                        CLAY_STRING("Text"),
                        CLAY__INIT(Clay_Color){ 105, 210, 231, 255 },
                        offscreen);
                } else {
                    if (currentElement->config.backgroundColor.a > 0) {
                        Clay__DebugElementConfigTypeLabelConfig config =
                            Clay__DebugGetElementConfigTypeLabel(
                                CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR);
                        Clay__RenderElementConfigTypeLabel(
                            config.label, config.color, offscreen);
                    }
                    if (currentElement->config.overlayColor.a > 0) {
                        Clay__DebugElementConfigTypeLabelConfig config =
                            Clay__DebugGetElementConfigTypeLabel(
                    ...
                    if (!Clay__MemCmp(
                            (const char *)&currentElement->config.cornerRadius,
            ...
            if (!(currentElement->isTextElement ||
                  (currentElementData &&
                   currentElementData->debugData->collapsed))) {
                for (int32_t i = currentElement->children.length - 1; i >= 0;
                     --i) {
                    Clay__int32_tArray_Add(
                        &dfsBuffer, currentElement->children.elements[i]);
                    context->treeNodeVisited
...
void Clay__RenderDebugView(void)
{
                       ...
                       .width = { .bottom = 1 } } })
    {
        ...
        CLAY(scrollId, { .layout = { .sizing = { CLAY_SIZING_GROW(0),
                                                 CLAY_SIZING_GROW(0) } },
                                       ...
                                       .width = { .betweenChildren = 1 } } })
            {
                CLAY_AUTO_ID(
                    { .layout = {
                          ...
                          .childAlignment = { .y = CLAY_ALIGN_Y_CENTER } } })
                {
                    CLAY_TEXT(CLAY_STRING("Element Configuration"),
                              infoTextConfig);
                    CLAY_AUTO_ID(
                        { .layout = {
                ...
                if (selectedItem->layoutElement->isTextElement) {
                    ...
                    CLAY(
                        CLAY_ID("Clay__DebugViewElementInfoSharedBody"),
                        { .layout = { .padding = attributeConfigPadding,
                                      .childGap = 8,
                                      .layoutDirection = CLAY_TOP_TO_BOTTOM } })
                    {
                        ...
                        {
                            CLAY_TEXT(
                                CLAY_STRING("Color & Radius"),
                                CLAY_TEXT_CONFIG(
                                    { .textColor = CLAY__DEBUGVIEW_COLOR_4,
                                      .fontSize = 16 }));
                        }
                        // .backgroundColor
                        if (selectedItem->layoutElement->config.backgroundColor
                                .a > 0) {
                            CLAY_TEXT(CLAY_STRING("Background Color"),
                                      infoTitleConfig);
                            Clay__RenderDebugViewColor(
                                selectedItem->layoutElement->config
                                    .backgroundColor,
                                infoTextConfig);
                        }
                        // .cornerRadius
                        if (!Clay__MemCmp(
                                (const char *)&selectedItem->layoutElement
                                    ->config.cornerRadius,
                                (const char *)&Clay__CornerRadius_DEFAULT,
                                sizeof(Clay_CornerRadius))) {
                        ...
                        // .overlayColor
                        if (selectedItem->layoutElement->config.overlayColor.a >
                            0) {
                            CLAY_TEXT(CLAY_STRING("Overlay Color"),
                                      infoTitleConfig);
                            Clay__RenderDebugViewColor(
                    ...
                    if (selectedItem->layoutElement->config.aspectRatio
                            .aspectRatio > 0) {
                                               ...
                                               CLAY_TOP_TO_BOTTOM } })
                        {
                            ...
                            // Aspect Ratio
                            CLAY(CLAY_ID(
                                     "Clay__DebugViewElementInfoAspectRatio"),
                                 {})
                            {
                                CLAY_TEXT(Clay__IntToString(
...
{
    Clay_Context fakeContext = { .maxElementCount =
...
void Clay_SetPointerState(Clay_Vector2 position, bool isPointerDown)
{
    ...
    for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1;
            ...
            if (currentElement->config.transition.handler) {
                for (int I = 0; I < context->transitionDatas.length; ++I) {
                    ...
                    if (data->elementId == currentElement->id) {
                        if (currentElement->config.transition
                                .interactionHandling ==
                            CLAY_TRANSITION_DISABLE_INTERACTIONS_WHILE_TRANSITIONING_POSITION) {
                            ...
                            currentElement->config.transition
                                .interactionHandling ==
                            CLAY_TRANSITION_ALLOW_INTERACTIONS_WHILE_TRANSITIONING_POSITION) {
                            if (data->state == CLAY_TRANSITION_STATE_EXITING) {
                                skipTree = true;
        ...
        if (found &&
            rootElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE &&
        ...
        if (data->transitionOut) {
            ...
            Clay_LayoutElement *newElement =
                Clay_LayoutElementArray_Set_DontTouchLength(
                    &context->layoutElements, nextIndex,
            ...
            while (bufferIndex < bfsBuffer.length) {
                ...
                for (int j = layoutElement->children.length - 1; j >= 0; --j) {
                    ...
                    Clay__int32_tArray_Set_DontTouchLength(
                        &context->layoutElementChildren, nextChildIndex,
                        nextIndex);
                    nextIndex--;
                    nextChildIndex--;
                }
                layoutElement->children.elements =
                    &context->layoutElementChildren
                         .internalArray[nextChildIndex + 1];
            }
        }
    }
};
    ...
    bool reparented)
{
    ...
    if (properties & CLAY_TRANSITION_PROPERTY_OVERLAY_COLOR) {
        currentElement->config.overlayColor =
            currentTransitionData.overlayColor;
    }
                ...
                if (config->exit.trigger ==
                    ...
                    while (bufferIndex < bfsBuffer.length) {
                        Clay_LayoutElement *layoutElement =
                            Clay_LayoutElementArray_GetCheckCapacity(
                                &context->layoutElements,
                                Clay__int32_tArray_GetValue(&bfsBuffer,
                                                            bufferIndex));
                        bufferIndex++;
                        int32_t firstChildSlot =
                            context->layoutElementChildren.length;
                        for (int j = 0; j < layoutElement->children.length;
                             ++j) {
                            Clay_LayoutElement *childElement =
                                Clay_LayoutElementArray_GetCheckCapacity(
                                    &context->layoutElements,
                            ...
                            Clay__int32_tArray_Add(
                                ...
                                *Clay__int32_tArray_GetCheckCapacity(
                                    ...
                                    childElementIndex));
                            Clay__int32_tArray_Add(
                                &bfsBuffer, context->layoutElements.length - 1);
                            if (newChildElement->isTextElement) {
                                newChildElement->textElementData.wrappedLines
                                    .length = 0;
                            }
                            Clay__int32_tArray_Add(
                                &context->layoutElementChildren,
                    ...
                    if (parentHashMapItem->generation > context->generation) {
                        ...
                        for (int j = 0; j < parentElement->children.length;
                             ++j) {
                            if (config->exit.siblingOrdering ==
                                    CLAY_EXIT_TRANSITION_ORDERING_NATURAL_ORDER &&
                                j == data->siblingIndex) {
                        ...
                        Clay__LayoutElementTreeRootArray_Add(
                            ...
                            CLAY__INIT(Clay__LayoutElementTreeRoot){
                                ...
                                .parentId =
                                    Clay__HashString(
                                        CLAY_STRING("Clay__RootContainer"), 0)
                                        .id,
                                .zIndex = 1,
                            });
                    }
    ...
    if (context->booleanWarnings.maxElementsExceeded) {
        ...
        if (context->transitionDatas.length > 0) {
            ...
            for (int i = 0; i < context->transitionDatas.length; ++i) {
                Clay__TransitionDataInternal *transitionData =
                    Clay__TransitionDataInternalArray_Get(
                        &context->transitionDatas, i);
                Clay_LayoutElement *currentElement =
                    transitionData->elementThisFrame;
                ...
                Clay_TransitionData oldTargetState =
                ...
                if (mapItem->appearedThisFrame) {
                    if (currentElement->config.transition.enter.setInitialState &&
                        ...
                        transitionData->initialState =
                            currentElement->config.transition.enter
                                .setInitialState(transitionData->targetState,
                    ...
                    if (properties &
                        ...
                        if (!Clay__MemCmp(
                                (char *)&oldTargetState.backgroundColor,
                    ...
                    if (transitionData->state == CLAY_TRANSITION_STATE_IDLE) {
                        ...
                        transitionComplete =
                            currentElement->config.transition.handler(CLAY__INIT(
                                Clay_TransitionCallbackArguments){
                                transitionData->state,
                                transitionData->initialState,
                                &transitionData->currentState, targetState,
            ...
            if (context->booleanWarnings.maxElementsExceeded) {
                ...
                Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                    .boundingBox = { context->layoutDimensions.width / 2 -
                                         59 * 4,
                                     context->layoutDimensions.height / 2, 0,
                                     0 },
                    .renderData = { .text = { .stringContents =
                                                  CLAY__INIT(Clay_StringSlice){
                                                      .length = message.length,
...
Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id)
{
    ...
    for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
        ...
        if (scrollContainerData->elementId == id.id) {
            if (!scrollContainerData
                     ->layoutElement) { // This can happen on the first frame before a scroll container is declared
                return CLAY__INIT(Clay_ScrollContainerData)
                    CLAY__DEFAULT_STRUCT;
            }
            return CLAY__INIT(Clay_ScrollContainerData){
                .scrollPosition = &scrollContainerData->scrollPosition,
                .scrollContainerDimensions = { scrollContainerData->boundingBox
                                                   .width,
                                               scrollContainerData->boundingBox
                                                   .height },
    ...
    if (arguments.properties & CLAY_TRANSITION_PROPERTY_BORDER_WIDTH) {
        arguments.current->borderWidth = CLAY__INIT(Clay_BorderWidth){
            ...
            .top = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.top,
                                        arguments.target.borderWidth.top,
                                        lerpAmount),
