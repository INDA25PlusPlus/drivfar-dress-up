...
static bool isMouseOverText(sfText* text, sfRenderWindow* window)
{
    ...
    return sfFloatRect_contains(&bounds, (float)mousePos.x, (float)mousePos.y);
}

int main(void)
{
    sfVideoMode mode = {800, 600, 32};
    sfRenderWindow* window = sfRenderWindow_create(mode, "Dress Up Skeleton", sfResize | sfClose, NULL);
    if (!window)
        return 1;

    /* Background image */
    sfTexture* backgroundTexture = sfTexture_createFromFile("cute_image.jpg", NULL);
    ...
    /* Music */
    sfMusic* music = sfMusic_createFromFile("nice_music.ogg"); /* Morrowind explore track, of course */
    if (!music) {
        sfFont_destroy(font);
        sfSprite_destroy(backgroundSprite);
        sfTexture_destroy(backgroundTexture);
    ...
    while (sfRenderWindow_isOpen(window))
    {
        ...
        while (sfRenderWindow_pollEvent(window, &event))
        {
            ...
            {
                if (currentPage == PAGE_DRESS_UP && isMouseOverText(arrowText, window))
                {
                    currentPage = PAGE_GRADING;
                }
                else if (currentPage == PAGE_GRADING && isMouseOverText(backArrowText, window))
                {
